...
#  get infput file names
source_archv_i=$1
echo "source archv in remap is ${source_archv_i}"

# look at inner nesting region

source ${BASEDIR}/REGION.src || { echo "Could not source ${BASEDIR}/REGION.src" ; exit 1 ; }
source ./EXPT.src || { echo "Could not source ./EXPT.src" ; exit 1 ; }
source ${NHCBINDIR}/common_functions.sh || { echo "Could not source ${NHCBINDIR}/common_functions.sh" ; exit 1 ; }

# find aspects of inner region
IR=$R
IE=$E
IX=$X
IT=$T

echo "NESTROOT = $NESTROOT"

# first get info from NEMO dir (outer nesting region)

source ${NESTROOT}/REGION.src || { echo "Could not source ${NESTROOT}/REGION.src" ; exit 1 ; }
source ${NESTROOT}/expt_${NESTEXPT}/EXPT.src || { echo "Could not source ${NESTROOT}/expt_${NESTEXPT}/EXPT.src" ; exit 1 ; }
# NEST experiment set in REGION.src file

# find aspects of outer region
OR=$R
OE=$E
OX=$X
OT=$T
...
echo "Inner region name : $IR"
echo "Inner region experiment: $IX"

# create directories
NEST=${BASEDIR}/nest/${IE}
EXPTNEW=${BASEDIR}/expt_${IX}

# directories from NEMO
D=${NESTROOT}/expt_${OX}/data
N=${NESTROOT}/subregion/${OE}
...
cd $D
echo ${NESTROOT}/topo/depth_${OR}_${OT}.b
echo ${BASEDIR}/topo/depth_${IR}_${IT}.b
echo "################"
#
#   From outer region
#
cp  ${NESTROOT}/topo/regional.grid.a           $D/regional.grid.a
cp  ${NESTROOT}/topo/regional.grid.b           $D/regional.grid.b
cp  ${NESTROOT}/topo/depth_${OR}_${OT}.a       $D/regional.depth.a
cp  ${NESTROOT}/topo/depth_${OR}_${OT}.b       $D/regional.depth.b
#
#   From inner region
#
cp  ${NESTROOT}/topo/ZL50.txt                  $N/ZL50.txt
cp  ${NESTROOT}/subregion/${IR}.gmap.a          $N/regional.gmap.a
cp  ${NESTROOT}/subregion/${IR}.gmap.b          $N/regional.gmap.b
cp  ${EXPTNEW}/blkdat.input                   $N/blkdat.input
cp  ${BASEDIR}/topo/regional.grid.a     $N/regional.grid.a
cp  ${BASEDIR}/topo/regional.grid.b     $N/regional.grid.b
cp  ${BASEDIR}/topo/depth_${IR}_${IT}.a   $N/regional.depth.a
cp  ${BASEDIR}/topo/depth_${IR}_${IT}.b   $N/regional.depth.b
#
...
cd $D
echo
echo "Succesfully created archive file: $1"
echo
echo
